...
import com.lianggao.whut.androidebook.Entity.HttpConfig;
...
public class HttpCaller {
    ...
    private Call getBuilder(String url, Header[] header, HttpResponseHandler responseCallback) {
        url=NetUtil.getMosaicParameter(url,httpConfig.getCommonField());//拼接公共参数
...
//		Log.i("ansen","url:"+url);
    ...
    /////////////////////
    public <T> List<T> postSyncList(Class<T> clazz, String url, List<NameValuePair> form) {
        return postSyncList(clazz,url,form,null);
    }
    //得到list对象返回
    public <T> List<T> postSyncList(Class<T> clazz, String url, List<NameValuePair> form, Header[] header) {
        if (checkAgent()) {
            return null;
        }
//		Log.i("ansen","url:"+url);
        ...
        try {
            ...
            if ( !TextUtils.isEmpty(result)) {
                ...
                Log.i("用户&",jsonArray.toString());
                String jsonstr=jsonArray.toString();
                for(int i=0;i<jsonArray.size();i++){
                    JSONObject jsonObject=jsonArray.getJSONObject(i);
                    Class <T> book= (Class<T>) JSONObject.toBean(jsonObject,clazz.getClass());
                    tList1.add((T) book);
                }
*/
        ...
        try {
            ...
            for(int i=form.size()-1;i>=0;i--){
                ...
                if(item.isFile()){//上传文件
                    ...
                    if (myFile.exists()){
                        String fileName = NetUtil.getFileName(item.getValue());
