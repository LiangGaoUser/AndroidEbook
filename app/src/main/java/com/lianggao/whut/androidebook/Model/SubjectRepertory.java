...
public class SubjectRepertory {
    ...
    public static List<MySubject> loadDefaultSubjects3() {
        //String json="[[\"2017-2018学年秋\", \"\", \"\", \"计算机组成原理\", \"\", \"\", \"\", \"\", \"刘静\", \"\", \"\", \"1周上\", 1, 1, 2, \"\", \"计算机综合楼106\", \"\"]]";
        String json="[['2017-2018学年秋', '\', '\', '计算机组成原理', '\', '\', '\', '\', '刘静', '\', '\', '1', 1, 1, 2, '\', '计算机综合楼106', '\']]";
        //[term,name,room,tearcher,weeklist,start,step,day,color,time]
        List<MySubject>mySubjectList=parse(json);
        for(int i=0;i<mySubjectList.size();i++){
            System.out.println("###############################  term  "+ mySubjectList.get(i).getTerm()+" name "+mySubjectList.get(i).getName()+" room "+mySubjectList.get(i).getRoom()+" teacher "+mySubjectList.get(i).getTeacher()+" time "+mySubjectList.get(i).getTime()+" day "+mySubjectList.get(i).getDay()+" start "+mySubjectList.get(i).getStart()+" step "+mySubjectList.get(i).getStep()+ "weeklist"+mySubjectList.get(i).getWeekList() );
        }
        return parse(json);
    }
