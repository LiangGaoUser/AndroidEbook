...
public class LoadMoreRecommendBookRecyclerView extends RecyclerView {

    private LinearLayoutManager mLinearLayoutManager;
    private LoadMoreBookAdapter mAdapter;
    private GridLayoutManager gridLayoutManager;

    public LoadMoreRecommendBookRecyclerView(Context context) {
        this(context,null);
    }

    public LoadMoreRecommendBookRecyclerView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs,0);
    }

    public LoadMoreRecommendBookRecyclerView(Context context, @Nullable AttributeSet attrs, int defStyle) {
    ...
    public void onScrolled(int dx, int dy) {
        ...
        if (endCompletelyPosition == mAdapter.getItemCount()-1){
            this.post(new Runnable() {
                @Override
                public void run() {
                    mAdapter.LoadMoreRecommendBook();
                }
            });
